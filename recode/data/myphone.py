...
class Dataset(base.Dataset):
    ...
    def __init__(self,opt,split="train",subset=None):
        self.raw_H,self.raw_W = 4000,3000
        super().__init__(opt,split)
        self.root = opt.data.root or "data/myphone"
    ...
    def get_camera(self,opt,idx):
        #self.focal = self.raw_W*4.2/(12.8/2.55) # old calculation for Iphone 12
        # pixel_size_mm = CCD_mm / self.raw_H # when the CCD width is known

        # camera: S21u, 180MP Wide camera, pixel size: Nona binning 2.4μm (12MP)
        focal_mm = 6.7
        pixel_size_mm = 2.4 / 1000
        self.focal = focal_mm / pixel_size_mm # fx = f / pixel_size
